...
export function getProfile(id) {
  return (dispatch) => {
    ...
    return axios.get(`${types.URL}/users/${id}`)
      .then((res) => {
        dispatch({
...
export function getUsers(offset = 0, limit = 9) {
  return (dispatch) => {
    ...
    return axios.get(`${types.URL}/users?limit=${limit}&offset=${offset}`)
      .then((res) => {
        dispatch({
...
export function searchUsers(query, offset = 0, limit = 9) {
  return (dispatch) => {
    ...
    return axios.get(`${types.URL}/users?q=${query}&limit=${limit}&offset=${offset}`)
      .then((res) => {
        dispatch({
...
export function saveUser(user) {
  ...
  return (dispatch) => {
    ...
    return axios.put(`${types.URL}/users/${user.id}`, user)
      .then((res) => {
...
export function deleteUser(id) {
  return (dispatch) => {
    ...
    return axios.delete(`${types.URL}/users/${id}`)
      .then(() => {
        dispatch({
