...
export function clientLogin(token, type) {
  ...
  const user = { id: decoded.user_id, roleId: decoded.role_id, username: decoded.username };
...
export function login(userDetails) {
  return (dispatch) => {
    ...
    return axios.post(`${types.URL}/users/login`, userDetails)
      .then((res) => {
...
export function saveUser(userDetails) {
  userDetails.full_name = userDetails.fullName;
  userDetails.role_id = userDetails.roleId;
  if (userDetails.id) {
    return (dispatch) => {
      dispatch(beginAjaxCall());

      return axios.put(`${types.URL}/users/${userDetails.id}`, userDetails)
        .then(() => {
          dispatch({
  ...
  return (dispatch) => {
    ...
    return axios.post(`${types.URL}/users`, userDetails)
      .then((res) => {
        const token = res.data.token;
